...
function buildIndex(seasons) {
  ...
  for (const s of seasons) {
    const schoolName = s.school_id?.toUpperCase();
    ...
    for (const r of roster) {
      const pid = r.player_id || `${s.school_id}_${(s.season||'').slice(0,4)}_${(r.number||'xx')}_${(r.player_name||'').toLowerCase().replace(/[^a-z0-9]+/g,'_')}`;
